...
public class MenuSort implements Comparable<MenuSort>
{
    ...
    public ArrayList<MenuVO> getMenuVOList(){
        ArrayList<MenuVO> menuVOList = new ArrayList<MenuVO>();
    ...
    public static ArrayList<MenuVO> sortShowMenu(ArrayList<MenuVO> menuVOList){
      //组装Map数据,将所有菜单全部放到Map中，并用菜单ID作为Key来标记，方便后面的各节点寻找父节点。
        ...
        for (Map.Entry<String, MenuSort> entry : dataMap.entrySet()) {
            ...
            if (menu.getParentId().trim().equals(root.getId())) {
        ...
        root.sortChildren();
        //返回排序后的菜单节点
        ArrayList<MenuVO> returnList = root.getMenuVOList();
